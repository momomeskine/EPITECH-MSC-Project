defmodule TimeManagerAPIWeb.WorkingtimeController do
  ...
  def create(conn, %{"userID" => userID, "workingtime" => workingtime_params}) do
    with {:ok, %Workingtime{} = workingtime} <- Workingtimes.create_workingtime(workingtime_params, userID) do
      conn
      |> put_status(:created)
  ...
  def search(conn, %{"id" => id} = params) do
    ...
    cond do
      startSearch != "" && endSearch != "" -> render(conn, "index.json", workingtimes: Workingtimes.search_workingtime(id, startSearch, endSearch))
      true -> render(conn, "show.json", workingtime: Workingtimes.get_workingtime!(id))
    end
  end
